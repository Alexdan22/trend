...
const CHART_RIGHT = WIDTH - PRICE_LABEL_WIDTH;

const CHART_TOP = HEADER_HEIGHT + 20;

const CHART_BOTTOM = HEIGHT - FOOTER_HEIGHT - 30;

//Helper functions

function getPriceRange(candles, entryPrice, stopLoss, takeProfit) {
  const highs = candles.map((c) => c.high);

  const lows = candles.map((c) => c.low);

  let maxPrice = Math.max(...highs, entryPrice, takeProfit);

  let minPrice = Math.min(...lows, stopLoss, entryPrice);
...
function priceToY(price, minPrice, maxPrice) {
  ...
  return (
    CHART_BOTTOM - ((price - minPrice) / (maxPrice - minPrice)) * usableHeight
  );
}

function drawLevel(ctx, price, color, label, minPrice, maxPrice) {
  ...
  ctx.fillText(`${label} ${price.toFixed(2)}`, CHART_RIGHT + 15, y - 8);
}
...
async function generateTradeChart({
  ...
  ctx.fillText(`${side} XAUUSD | ${event}`, 30, 40);
  ...
  ctx.moveTo(CHART_RIGHT, CHART_TOP);
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    const x = CHART_LEFT + i * candleWidth;
    ...
    ctx.fillRect(x + 1, bodyTop, Math.max(2, candleWidth - 2), bodyHeight);
  }

  drawLevel(ctx, entryPrice, "#ffffff", "ENTRY", minPrice, maxPrice);

  drawLevel(ctx, takeProfit, "#3b82f6", "TP", minPrice, maxPrice);

  drawLevel(ctx, stopLoss, "#ef4444", "SL", minPrice, maxPrice);
